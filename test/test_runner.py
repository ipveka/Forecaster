...
def test_frequency(freq, model="LGBM"):
    ...
    try:
        ...
        try:
            ...
            # Extract RMSE values
            baseline_rmse = (
                metrics["RMSE"]["baseline_sales_ma_13"]
                if metrics
                and "RMSE" in metrics
                and "baseline_sales_ma_13" in metrics["RMSE"]
                else None
            )
            prediction_rmse = (
                metrics["RMSE"]["prediction"]
                if metrics and "RMSE" in metrics and "prediction" in metrics["RMSE"]
                else None
            )
            ...
            print(
                f"   Data shape: {result_df.shape[0]} rows × {result_df.shape[1]} columns"
            )
            if improvement:
