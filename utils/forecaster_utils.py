    ...
    for date in dates:
        for product_id in product_ids:
            for store_id in store_ids:
                ...
                sales = float(
                    base_value * weekday_factor * store_factor
...
def visualize_forecasts_by_cutoff(
    df, outputs_dir, target_col="sales", pred_col="prediction"
):
    """
    ...
    if len(cutoffs) > 0:
        ...
        baseline_cols = [
            col for col in df.columns if col.startswith(f"baseline_{target_col}_ma_")
        ]
        ...
        for i, cutoff_date in enumerate(cutoffs):
            ...
            if "sample" not in cutoff_df.columns:
                ...
                cutoff_df["sample"] = np.where(
                    cutoff_df["date"] <= cutoff_timestamp, "train", "test"
                )

            # Group by date and sample to get sum by train/test split
            data_by_sample = (
                cutoff_df.groupby(["date", "sample"])[[target_col, pred_col]]
                .sum()
                .reset_index()
            )
            if baseline_col and baseline_col in cutoff_df.columns:
                ...
                baseline_data = (
                    cutoff_df.groupby(["date", "sample"])[baseline_col]
                    .sum()
                    .reset_index()
                )
                # Merge baseline data with main data
                data_by_sample = pd.merge(
                    data_by_sample, baseline_data, on=["date", "sample"], how="left"
                )
...
def save_results(df, metrics, freq, outputs_dir):
    ...
    if metrics:
        ...
        if len(cutoffs) > 1:
            ...
            cutoff_metrics_df = pd.DataFrame.from_dict(
                metrics_by_cutoff, orient="index"
            )
