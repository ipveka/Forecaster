...
class ForecastRequest(BaseModel):
    ...
    # Data
    data: List[Dict[str, Any]] = Field(..., description="Data in JSON format (list of records)")
    
    # Required parameters
    date_col: str = Field(..., description="Name of the date column")
    group_cols: List[str] = Field(..., description="List of columns to group by")
    signal_cols: List[str] = Field(..., description="List of signal columns to forecast")
    
    # Data preparation parameters
    target: Optional[str] = Field(None, description="Target column (if None, uses the first signal column)")
    horizon: int = Field(4, description="Forecasting horizon")
    freq: Optional[str] = Field(None, description="Frequency of the data ('D', 'W', 'M')")
    n_cutoffs: int = Field(1, description="Number of cutoffs for backtesting")
    complete_dataframe: bool = Field(True, description="Whether to fill in missing dates")
    ...
    fe_window_size: Union[int, List[int]] = Field((4, 13), description="Window sizes for feature engineering")
    lags: Union[int, List[int]] = Field((4, 13), description="Lag values for creating lag features")
    ...
    # Baseline parameters
    baseline_types: List[str] = Field(['MA'], description="Types of baselines to create")
    bs_window_size: int = Field(13, description="Window size for moving average baseline")
    
    # Forecaster parameters
    training_group: Optional[str] = Field(None, description="Column to use for training groups")
    model: str = Field('LGBM', description="Model to use for forecasting")
    tune_hyperparameters: bool = Field(False, description="Whether to tune hyperparameters")
    search_method: str = Field('halving', description="Search method for hyperparameter tuning")
    scoring: str = Field('neg_mean_squared_log_error', description="Scoring metric for hyperparameter tuning")
    ...
    remove_outliers: bool = Field(False, description="Whether to remove outliers in forecasting")
    outlier_column: Optional[str] = Field(None, description="Column from which to remove outliers")
    lower_quantile: float = Field(0.025, description="Lower quantile for outlier removal")
    upper_quantile: float = Field(0.975, description="Upper quantile for outlier removal")
    ts_decomposition: bool = Field(False, description="Whether to use time series decomposition")
    baseline_col: Optional[str] = Field(None, description="Baseline column to use for comparison")
    use_guardrail: bool = Field(False, description="Whether to use guardrail limits")
    guardrail_limit: float = Field(2.5, description="Limit value for guardrail adjustments")
    use_weights: bool = Field(False, description="Whether to use weights in training")
    use_parallel: bool = Field(True, description="Whether to run predictions in parallel")
    num_cpus: Optional[int] = Field(None, description="Number of CPUs to use for parallel processing")
    
    # Evaluator parameters
    eval_group_col: Optional[str] = Field(None, description="Column to group by for evaluation")
    eval_group_filter: Optional[List[str]] = Field(None, description="Filter for groups in evaluation")
...
async def forecast(request: ForecastRequest, authorized: bool = Depends(get_api_key)):
    ...
    try:
        ...
        fe_window_size = tuple(request.fe_window_size) if isinstance(request.fe_window_size, list) else request.fe_window_size
