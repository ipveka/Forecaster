...
def upload_to_supabase(df, table_name):
    ...
    if supabase_url == "your-supabase-url" or supabase_key == "your-supabase-key":
        logging.error(
            "Supabase credentials not configured. Please update the .env file."
        )
        return
    ...
    try:
        ...
        for i in range(0, len(records), batch_size):
            ...
            response = requests.post(
                upload_url, headers=headers, data=json.dumps(batch)
            )
            response.raise_for_status()
            logging.info(
                f"Uploaded batch {i//batch_size + 1}/{(len(records)-1)//batch_size + 1}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate sample data and upload to database"
    )
    parser.add_argument(
        "--db_config",
        type=str,
    ...
    print(
        f"Daily data shape: {daily_data.shape[0]} rows × {daily_data.shape[1]} columns"
    )
    print(
        f"Weekly data shape: {weekly_data.shape[0]} rows × {weekly_data.shape[1]} columns"
    )
    print(
        f"Monthly data shape: {monthly_data.shape[0]} rows × {monthly_data.shape[1]} columns"
    )
    print("=" * 80)
